...
sorted_y = sorted(points, lambda point: point.y)

min_y = sorted_y[0].y
max_y = sorted_y[::-1][0].y

sorted_x = sorted(points, lambda point: point.x)
...
for x, y in itertools.product(range(min_x, max_x + 1),
                              range(min_y, max_y + 1)):
    point = Point2D(x, y)
...
aoc_part_1_answer = sorted(grid_points.items(),
